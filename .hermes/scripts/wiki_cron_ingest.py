#!/usr/bin/env python3
"""Automated Wiki Ingestor for Hermès cron jobs to fetch meeting transcripts (Granola) and other sources, indexing them into the Obsidian wiki."""
...
def ingest_source(source_name: str) -> dict:
    ...
    result = {
        ...
        "status": "success",
        "files_created": 0,
        "message": f"Started {source_name} ingestion...",
    }
    ...
    try:
        if config["source_type"] == "api":
            ...
            if not endpoint:
                result["message"] += f" — skip execution (no API endpoint)"
        elif config["source_type"] == "web":
            pattern = config.get("url_pattern")
            result["message"] = f"Web source: {pattern} — requires browser auto-"
